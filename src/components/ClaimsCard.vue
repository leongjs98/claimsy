<template>
  <div class="mb-8 flex gap-6 px-4 sm:px-8 lg:px-14">
    <RouterLink to="/employee/claim/expenses"
    ...
    >
      ...
      <div class="mt-2 font-semibold text-theme-300">
        Claims
      </div>
    </RouterLink>
    <RouterLink to="/employee/claim/approved"
    ...
    >
      ...
      <div class="mt-2 font-semibold text-[#2A9D8F]">
        Approved
      </div>
    </RouterLink>
    <RouterLink to="/employee/claim/rejected"
...
const approvedCount = computed(() => {
  const ids = new Set(props.expenses.filter(e => e.Status === 'Approved').map(e => e.ClaimID));
  return ids.size;
});

const rejectedCount = computed(() => {
  const ids = new Set(props.expenses.filter(e => e.Status === 'Rejected').map(e => e.ClaimID));
  return ids.size;
});
</script>