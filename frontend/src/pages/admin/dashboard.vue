  ...
  <div class="mx-auto my-14 w-full max-w-6xl bg-gray-100">
    ...
    <div class="grid grid-cols-2 gap-6 px-4 sm:px-8 lg:px-14">
      ...
      <div class="rounded-lg bg-white p-6 shadow-xl">
        <h2
          class="mb-4 rounded-lg bg-blue-50 p-1 text-center text-lg font-semibold text-theme-300"
        >
          Category Type
        </h2>
...
<script setup>
  ...
  const totalApprovedClaims = claims.filter(
    (c) => c.status === "approved",
  ).length;
  const totalRejectedClaims = claims.filter(
    (c) => c.status === "rejected",
  ).length;
  ...
  claims.forEach((c) => {
    categoryAmountTotals[c.category] =
      (categoryAmountTotals[c.category] || 0) + c.amount;
  });
  ...
  Chart.register({
    ...
    afterDraw(chart) {
      ...
      ctx.fillText(
        "RM " +
          totalAmountAll.toLocaleString("en-MY", { minimumFractionDigits: 2 }),
        width / 2,
        height / 2 + 8,
      );
    },
