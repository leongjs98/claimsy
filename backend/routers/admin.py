...
async def list_invoices(db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Internal Server Error: Could not retrieve invoices. {str(e)}")

@router.get(
    "/claim/details/{invoice_id}", response_model=Invoice)
async def get_invoice_by_id(invoice_id: int, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Internal Server Error: Could not retrieve invoice. {str(e)}")